...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Limpieza de gráficos al desmontar el componente
    return () => {
      if (chart1Ref.current?.chartInstance) chart1Ref.current.chartInstance.destroy();
      if (chart2Ref.current?.chartInstance) chart2Ref.current.chartInstance.destroy();
      if (chart3Ref.current?.chartInstance) chart3Ref.current.chartInstance.destroy();
    };
  }, []);
    ...
    transactions.forEach((transaction) => {
      if (transaction.type === 'expense') {
        ...
        if (transactionDate >= startOfWeek && transactionDate <= now) {
          ...
          dailyExpenses[dayIndex] += parseFloat(transaction.amount) || 0;
        }
      }
    });
  ...
  const doughnutData = useMemo(() => {
    ...
    transactions.forEach((transaction) => {
      if (transaction.type === 'expense') {
        const category = transaction.category?.toLowerCase() || 'Sin categoría';
  ...
  const handleSaveGoals = async () => {
    ...
    if (user) {
      try {
        ...
        console.error("Dashboard: Error al guardar metas de ahorro:", error);
      }
    }
  };
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    if (user) {
      try {
        ...
        console.error("Dashboard: Error al agregar transacción:", error);
      }
    }
  };
