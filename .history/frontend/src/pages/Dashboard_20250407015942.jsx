...
const Dashboard = () => {
    ...
    transactions.forEach((transaction) => {
      if (transaction.type === "expense") {
        ...
        if (transactionDate >= startOfWeek && transactionDate <= now) {
          ...
          dailyExpenses[dayIndex] += parseFloat(String(transaction.amount));
        }
      }
    });
    ...
    transactions.forEach((transaction) => {
      if (transaction.type === "expense") {
        ...
        categoryTotals[category] =
          (categoryTotals[category] || 0) + parseFloat(String(transaction.amount));
      }
    });
  ...
  const totalExpenses =
    doughnutData.datasets[0]?.data.reduce((a, b) => a + b, 0) || 0;
        ...
        <div className="grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-3">
          <div className="space-y-4 md:space-y-6 lg:col-span-1">
            <motion.div
              ...
              style={{
                background:
                  "linear-gradient(135deg, #d8b4fe, #fed7aa, #f5f5f5)",
              }}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
            ...
            >
              ...
              <p className="text-sm text-gray-600 mb-2">
                Ingresos y gastos de la semana actual
              </p>
              <div className="h-40">
                <Bar
                  ref={chart1Ref}
                      ...
                      tooltip: {
                        callbacks: {
                          label: function (context) {
                            ...
                            return `${day}: ${formatCurrency(
                              value
                            )} (Gastos: ${formatCurrency(value)}, Ingresos: ${formatCurrency(
                              0
                            )})`;
                          },
                        },
              ...
              <p className="text-sm text-gray-600 mt-4 flex items-center gap-1">
                Mayor movimiento el{" "}
                {
                  barData.datasets[0].data.reduce(
                    (max, curr, i) =>
                      curr > max.value
                        ? { day: barData.labels[i], value: curr }
                        : max,
                    { day: "", value: 0 }
                  ).day
                }
          ...
          <div className="space-y-4 md:space-y-6 lg:col-span-1">
            <motion.div
              ...
              style={{
                background:
                  "linear-gradient(135deg, #f3e8ff, #fef3e8, #ffffff)",
              }}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
            ...
            >
              <button
                className="flex justify-between items-center w-full focus:outline-none focus:ring-2 focus:ring-purple-500"
                onClick={handleSavingsGoalsClick}
                aria-label="Configurar metas de ahorro"
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Define tus objetivos de ahorro mensual y anual
                  </p>
                </div>
                <svg
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 w-48 flex-shrink-0 md:w-full md:max-w-[16rem]">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Meta: {formatCurrency(savingsGoals.dailyLimit)}
                    </p>
                  </div>
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 w-48 flex-shrink-0 md:w-full md:max-w-[16rem]">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Meta: {formatCurrency(savingsGoals.weeklyLimit)}
                    </p>
                  </div>
                  <div className="relative w-12 h-12">
                    <svg className="w-full h-full" viewBox="0 0 36 36">
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 w-48 flex-shrink-0 md:w-full md:max-w-[16rem]">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Meta: {formatCurrency(savingsGoals.monthlyLimit)}
                    </p>
                  </div>
                  <div className="relative w-12 h-12">
                    <svg className="w-full h-full" viewBox="0 0 36 36">
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 w-48 flex-shrink-0 md:w-full md:max-w-[16rem]">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Meta: {formatCurrency(savingsGoals.annualLimit)}
                    </p>
                  </div>
                  <div className="relative w-12 h-12">
                    <svg className="w-full h-full" viewBox="0 0 36 36">
                      ...
                      <path
                        ...
                        strokeDasharray={`${calculatePercentage(
                          expenses.year,
                          savingsGoals.annualLimit
                        )}, 100`}
                      />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-sm text-gray-500">
                        {calculatePercentage(expenses.year, savingsGoals.annualLimit)}%
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-100 w-48 flex-shrink-0 md:w-full md:max-w-[16rem]">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Meta: {formatCurrency(savingsGoals.savingsGoal)}
                    </p>
                  </div>
                  <div className="relative w-12 h-12">
                    <svg className="w-full h-full" viewBox="0 0 36 36">
                  ...
                  {doughnutData.labels.map((label, index) => (
                    ...
                    >
                      ...
                      <span className="text-gray-800 font-semibold">
                        {formatCurrency(doughnutData.datasets[0].data[index])}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  Días de recordatorio para tareas pendientes
                </label>
                <input
                  type="number"
                  id="reminderDays"
